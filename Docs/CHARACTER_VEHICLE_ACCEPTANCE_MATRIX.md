...
| CV-DRIVE-006 | Roll or invert the vehicle, continue applying input, and attempt recovery using only currently exposed OWS controls. | Possession remains valid and physics remain finite. Current OWS controls expose no reset. The target developer-optional reset capability remains `NR` until its Vehicle-only contract and issue #47 evidence pass; this protected current-behavior row does not require it to be enabled. | Video plus the configured reset state and issue #47 evidence when applicable. |
