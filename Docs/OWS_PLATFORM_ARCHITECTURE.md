...
| Character | Evolve the complete GASPALS fork in place | Bodily locomotion and execution, dynamic-support movement, stance, balance/grip capability, traversal, animation, fall/roll/recovery, Character camera behavior, Character input assets, authoring, validation, and tests; the public contract is co-designed through the selected Mobility program |
| Vehicle | Evolve the complete KinetiForge fork in place | Vehicle simulation and force application, driving, point kinematics, seats/control roles, doors/portals, exterior zones/anchors, Vehicle-specific input and settings, authoring, validation, and tests; the public contract is co-designed through the selected Mobility program |
| Character–Vehicle Mobility integration | Narrow first-party integration plugin family; exact descriptor name deferred | Reusable relationship/load/collision coordination, dynamic support/riding, boarding, occupancy/control handoff, stopped exit, moving bailout, recovery/re-entry, exterior traversal, Grip, moving entry, same-Vehicle infiltration, reservations, and multiplayer orchestration using only participating domains' public contracts; it calculates/deduplicates coupling while Character executes bodily motion and Vehicle applies forces. Cross-Vehicle routes are representable now but runtime certification is later. |
...
A domain is split into another plugin only when independent installation, activation, cooking, or dependency direction genuinely requires it. Character–Vehicle Mobility integration qualifies because placing it inside either domain would make the whole host plugin depend on the other domain.
...
- The selected [OWS Mobility program](OWS_MOBILITY_PROGRAM_CHARTER.md) co-designs the Character, Vehicle, collision/force-response, and cross-domain relationship contracts before implementation freezes either public surface.
- Character–Vehicle dynamic support, boarding, occupancy/control handoff, exit, bailout, recovery, re-entry, exterior traversal, grip, moving entry, infiltration, transfer, reservation, and authority orchestration migrate from the project module only after #147, #107, #108, and #21 are accepted and their required public contracts exist.
...
| 5. Coordinated Mobility domain forks | [#147](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/147), [#107](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/107), [#108](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/108) | Co-design compatible public contracts, then evolve the complete Character and Vehicle forks in place as independent domains | Aurora selected Mobility; #147/#107/#108 and the complete matrix are accepted; each implementation issue is separately rewritten, ready, and claimed | Complete inherited-capability regression, independent clean-host build/load, compatible OWS-facing Supported surfaces, point-kinematic evidence, and domain cook/package evidence | Losing capabilities, freezing incompatible public contracts, selective migration, or reintroducing the rejected external-provider model |
| 6. Dynamic Character–Vehicle Mobility integration | [#120](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/120) | Coordinate the decomposed same-Vehicle implementation family—support/riding, boarding, occupancy/control handoff, exit/bailout/recovery, exterior traversal, Grip, moving entry, infiltration, reservations, and tests—then migrate only after protected parity; author/validate cross-Vehicle routes without claiming runtime support | #147/#107/#108/#21 accepted and required public contracts implemented; each #120 child separately ready and claimed | Character and Vehicle build independently; existing cycles remain protected; the complete declared same-Vehicle matrix passes through public contracts | Dependency cycles, duplicate domain state, unsafe teleport transitions, network divergence, or damage to accepted entry/exit/bailout behavior |
...
City implementation continues independently under its accepted contract; only later alignment with Platform, Stage 2, or an actual Mobility-facing road/surface dependency waits for those interfaces. Aurora selected OWS Mobility for Stage 3 through #147; Systemic Hacking remains gated and this selection does not alter or open Roadmap Stages 4 or 5. Their existing gates remain controlling.
...
- Treating the accepted Stage 3 Mobility target artifacts as permission to implement or as evidence of implementation.
- Replacing the accepted first-slice CMC/current-Vehicle foundations, or changing another foundation's detailed role, without the controlling accepted contract and separately authorized migration evidence.
...
- Exact Mobility child decomposition, symbols, file destinations, migrations, measured technology/tuning values, later Hacking opening, and the first Living World slice; the accepted dependency order itself is no longer deferred.
