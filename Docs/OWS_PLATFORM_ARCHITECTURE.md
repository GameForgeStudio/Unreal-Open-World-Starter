...
| Character | Evolve the complete GASPALS fork in place | Bodily locomotion, dynamic-support movement, stance, balance/grip capability, traversal, animation, fall/roll/recovery, character camera behavior, character input assets, authoring, validation, and tests; the public contract is co-designed through the selected Mobility program |
| Vehicle | Evolve the complete KinetiForge fork in place | Vehicle simulation, driving, point kinematics, seats/control roles, doors/portals, exterior zones/anchors, vehicle-specific input and settings, authoring, validation, and tests; the public contract is co-designed through the selected Mobility program |
| Character–Vehicle Mobility integration | Narrow first-party integration plugin family; exact descriptor name deferred | Reusable dynamic support/riding relationships, boarding, occupancy/control handoff, stopped exit, moving bailout, recovery/re-entry, exterior traversal, grip, moving entry, infiltration, transfers, reservations, and multiplayer orchestration using only participating domains' public contracts |
...
| 6. Dynamic Character–Vehicle Mobility integration | [#120](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/120) | Implement and migrate the accepted relationship system—support/riding, boarding, occupancy/control handoff, exit/bailout/recovery, exterior traversal, grip, moving entry, infiltration, transfers, reservations, and tests—into its narrow integration family | #147/#107/#108/#21 accepted and required public contracts implemented; #120 rewritten to the final matrix, separately ready, and claimed | Character and Vehicle build independently; existing cycles remain protected; the complete declared Mobility matrix passes through public contracts | Dependency cycles, duplicate domain state, unsafe teleport transitions, network divergence, or damage to accepted entry/exit/bailout behavior |
...
- Treating the selected Stage 3 Mobility program as permission to implement or as acceptance of its still-open detailed architecture.
...
- The exact post-architecture Mobility implementation order, detailed technology/tuning choices, later Hacking opening, and the first Living World slice.
