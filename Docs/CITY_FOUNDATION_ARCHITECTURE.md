...
This remains the accepted City subsystem contract under the package-wide [OWS Platform Architecture and Composition Contract](OWS_PLATFORM_ARCHITECTURE.md), accepted [OWS Shared Gameplay Spine, Authority, and Persistence Contract](OWS_GAMEPLAY_SPINE_ARCHITECTURE.md), and [OWS Product Architecture Roadmap](OWS_ARCHITECTURE_ROADMAP.md). The accepted Stage 1 ownership, dependency, content, profile, settings, cooking, compatibility, and conformance rules apply wherever they do not conflict with this City contract. Stage 2 supplies shared player/entity identity, action, server, and persistence semantics; City remains controlling for source provenance, recipe epochs, logical cells, feature identity generation, Actor promotion, and cache behavior. A conflict returns to Aurora for an explicit decision rather than silently changing either contract. City's authority invariants do not themselves implement the shared gameplay spine.

City-owned authoritative Actor/state promotion and demotion implementation is tracked by [#133](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/133). The shared World Partition/City persistence adapter is tracked by [#139](https://github.com/GameFusi/Unreal-Open-World-Starter/issues/139); neither issue absorbs the other's ownership.
