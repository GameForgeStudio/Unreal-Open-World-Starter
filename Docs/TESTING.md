...
The full capture records the guardrail self-test plus phase-specific build, smoke, automation, UAT cook exit, package-output oracle, manifest, and cleanup results. Packaged launch remains explicitly `BLOCKED` and owned by issue #17 until a deterministic oracle exists. Until the full command runs against the exact recorded commit, Unreal-backed results are `NR`; static capture is not a substitute. The self-contained JSON and sibling `.sha256` sidecar must contain no secret values, especially the credential tracked by issue #114. External tools may transiently write into isolated task scratch, which is sanitized after each phase and mandatorily deleted; none of that scratch is retained or used to modify a repository file merely to make a guardrail pass.
