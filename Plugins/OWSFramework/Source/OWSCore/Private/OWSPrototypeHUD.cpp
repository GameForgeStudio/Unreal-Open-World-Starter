...
void AOWSPrototypeHUD::DrawHUD()
{
	...
	const AOWSPrototypeCharacter* Character = Cast<AOWSPrototypeCharacter>(PlayerOwner->GetPawn());
	if (!Character)
	{
		return;
	}
	...
	float Y = 24.0f;
	DrawStatusLine(TEXT("OWS FULL-STACK PROTOTYPE"), Y, FLinearColor(0.55f, 0.9f, 1.0f));
	DrawStatusLine(Character->GetPrototypeStatus(), Y, FLinearColor::White);

	if (const UOWSPrototypeAttributeSet* Attributes = Character->GetPrototypeAttributes())
	{
		DrawStatusLine(
	...
	if (UOWSPrototypeInventorySystemComponent* Inventory =
		Character->FindComponentByClass<UOWSPrototypeInventorySystemComponent>())
	{
...
void AOWSPrototypeHUD::DrawStatusLine(
	const FString& Text,
	float& Y,
