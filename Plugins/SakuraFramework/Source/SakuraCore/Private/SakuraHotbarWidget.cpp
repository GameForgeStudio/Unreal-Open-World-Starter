...
void USakuraHotbarWidget::Configure(
	const ESakuraHotbarCellShape InShape,
...
void USakuraHotbarWidget::SetLayer(const ESakuraHotbarLayer InLayer)
{
	SetVisibility(InLayer == ESakuraHotbarLayer::None
		? ESlateVisibility::Collapsed
		: ESlateVisibility::HitTestInvisible);
}

void USakuraHotbarWidget::SetPressedSlot(const ESakuraHotbarInput Input)
{
...
void USakuraHotbarWidget::NativeConstruct()
{
...
void USakuraHotbarWidget::BuildCross()
{
	...
	for (int32 Index = 0; Index < 8; ++Index)
	{
		...
		FSlateBrush Brush;
		Brush.DrawAs = Shape == ESakuraHotbarCellShape::Circle ? ESlateBrushDrawType::RoundedBox : ESlateBrushDrawType::Box;
		Brush.TintColor = FSlateColor(FLinearColor(0.06f, 0.08f, 0.12f, 0.78f));
		if (Shape == ESakuraHotbarCellShape::Circle)
		{
