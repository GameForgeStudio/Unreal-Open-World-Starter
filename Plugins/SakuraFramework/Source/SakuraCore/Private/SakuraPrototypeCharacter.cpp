...
const FName ASakuraPrototypeCharacter::PrototypeSaveSlot(TEXT("SakuraPrototypeCheckpoint"));

ASakuraPrototypeCharacter::ASakuraPrototypeCharacter()
{
	...
	AttributeSet = CreateDefaultSubobject<USakuraPrototypeAttributeSet>(TEXT("AttributeSet"));
	InventorySystem = CreateDefaultSubobject<USakuraPrototypeInventorySystemComponent>(TEXT("InventorySystem"));
	EquipmentSystem = CreateDefaultSubobject<USakuraPrototypeEquipmentSystemComponent>(TEXT("EquipmentSystem"));
	InventoryBridge = CreateDefaultSubobject<USakuraInventoryBridgeComponent>(TEXT("InventoryBridge"));

	// These exact assets are created idempotently by the editor command
	// Sakura.GeneratePrototypeAssets. The first generator run also wires the
	...
	InventorySystem->PrototypeSwordDefinition = TSoftObjectPtr<USigilItemDefinition>(
		FSoftObjectPath(TEXT("/Game/SakuraPrototype/Data/DA_Item_DemoSword.DA_Item_DemoSword")));
	InventorySystem->PrototypePotionDefinition = TSoftObjectPtr<USigilItemDefinition>(
		FSoftObjectPath(TEXT("/Game/SakuraPrototype/Data/DA_Item_DemoPotion.DA_Item_DemoPotion")));
...
UAbilitySystemComponent* ASakuraPrototypeCharacter::GetAbilitySystemComponent() const
{
	return AbilitySystem;
}

void ASakuraPrototypeCharacter::BeginPlay()
{
	Super::BeginPlay();
...
void ASakuraPrototypeCharacter::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (HasAuthority())
	{
		if (USaveManager* SaveManager = USaveManager::Get(this))
...
void ASakuraPrototypeCharacter::PossessedBy(AController* NewController)
{
	Super::PossessedBy(NewController);
	InitializeAbilityActorInfo();
}

void ASakuraPrototypeCharacter::OnRep_Controller()
{
	Super::OnRep_Controller();
	InitializeAbilityActorInfo();
}

void ASakuraPrototypeCharacter::PawnClientRestart()
{
...
void ASakuraPrototypeCharacter::InitializeAbilityActorInfo()
{
	if (AbilitySystem)
	{
...
void ASakuraPrototypeCharacter::InitializePrototypeSystems()
{
...
void ASakuraPrototypeCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void ASakuraPrototypeCharacter::AddInputContexts()
{
...
void ASakuraPrototypeCharacter::Move(const FInputActionValue& Value)
{
...
void ASakuraPrototypeCharacter::Look(const FInputActionValue& Value)
{
	const FVector2D Input = Value.Get<FVector2D>();
...
void ASakuraPrototypeCharacter::StartJump() { Jump(); }
void ASakuraPrototypeCharacter::StopJump() { StopJumping(); }

USigilItemInstance* ASakuraPrototypeCharacter::FindFirstItem(const bool bEquipped, const bool bUseItem) const
{
	...
	for (const FSigilItemInfo& Info : Items)
	{
		...
		if (bUseItem ? Info.Item->FindFragmentByClass<USakuraItemFragment_UseEffect>() != nullptr
...
void ASakuraPrototypeCharacter::RequestEquipDemoItem()
{
	if (InventoryBridge) InventoryBridge->RequestEquipItem(FindFirstItem(false, false));
}

void ASakuraPrototypeCharacter::RequestUnequipDemoItem()
{
	if (InventoryBridge) InventoryBridge->RequestUnequipItem(FindFirstItem(true, false));
}

void ASakuraPrototypeCharacter::RequestUseDemoItem()
{
	if (InventoryBridge) InventoryBridge->RequestUseItem(FindFirstItem(false, true));
}

void ASakuraPrototypeCharacter::RequestActivateEquipmentAbility()
{
	if (AbilitySystem) AbilitySystem->TryActivateAbilityByClass(USakuraPrototypeGameplayAbility::StaticClass());
}

void ASakuraPrototypeCharacter::RequestSaveGroundedCheckpoint()
{
	if (HasAuthority()) SaveGroundedCheckpointAuthority(); else ServerSaveGroundedCheckpoint();
}

void ASakuraPrototypeCharacter::RequestLoadGroundedCheckpoint()
{
	if (HasAuthority()) LoadGroundedCheckpointAuthority(); else ServerLoadGroundedCheckpoint();
}

void ASakuraPrototypeCharacter::RequestAdjustPrototypeScalar(const float Delta)
{
	if (HasAuthority()) AdjustPrototypeScalarAuthority(Delta); else ServerAdjustPrototypeScalar(Delta);
}

void ASakuraPrototypeCharacter::IncreasePrototypeScalar()
{
	RequestAdjustPrototypeScalar(1.0f);
}

void ASakuraPrototypeCharacter::DecreasePrototypeScalar()
{
	RequestAdjustPrototypeScalar(-1.0f);
}

bool ASakuraPrototypeCharacter::ServerSaveGroundedCheckpoint_Validate() { return true; }
void ASakuraPrototypeCharacter::ServerSaveGroundedCheckpoint_Implementation() { SaveGroundedCheckpointAuthority(); }
bool ASakuraPrototypeCharacter::ServerLoadGroundedCheckpoint_Validate() { return true; }
void ASakuraPrototypeCharacter::ServerLoadGroundedCheckpoint_Implementation() { LoadGroundedCheckpointAuthority(); }
bool ASakuraPrototypeCharacter::ServerAdjustPrototypeScalar_Validate(const float Delta)
{
	return FMath::IsFinite(Delta) && FMath::Abs(Delta) <= 10.0f;
}
void ASakuraPrototypeCharacter::ServerAdjustPrototypeScalar_Implementation(const float Delta)
{
	AdjustPrototypeScalarAuthority(Delta);
}

void ASakuraPrototypeCharacter::SaveGroundedCheckpointAuthority()
{
...
void ASakuraPrototypeCharacter::LoadGroundedCheckpointAuthority()
{
	USaveManager* Manager = USaveManager::Get(this);
	if (!Manager || Manager->IsSavingOrLoading() || !Manager->IsSlotSaved(PrototypeSaveSlot))
	{
		SetPrototypeStatus(TEXT("No loadable Sakura prototype checkpoint is available."));
		return;
	}
...
void ASakuraPrototypeCharacter::AdjustPrototypeScalarAuthority(const float Delta)
{
...
void ASakuraPrototypeCharacter::OnSaveFinished(const FSELevelFilter& Filter, const bool bError)
{
	USaveManager* Manager = USaveManager::Get(this);
...
void ASakuraPrototypeCharacter::OnLoadFinished(const FSELevelFilter& Filter, const bool bError)
{
	USaveManager* Manager = USaveManager::Get(this);
	const USaveSlot* ActiveSlot = Manager ? Manager->GetActiveSlot() : nullptr;
	if (!ActiveSlot || ActiveSlot->Name != PrototypeSaveSlot)
...
void ASakuraPrototypeCharacter::SetPrototypeStatus(const FString& NewStatus)
{
	PrototypeStatus = NewStatus;
	UE_LOG(LogSakuraCore, Log, TEXT("%s"), *PrototypeStatus);
	ForceNetUpdate();
}

void ASakuraPrototypeCharacter::SetVehicleSeatedPresentation(
	const bool bSeated)
{
	if (!HasAuthority())
	{
...
void ASakuraPrototypeCharacter::OnRep_VehicleSeatedPresentation()
{
	ApplyVehicleSeatedPresentation();
}

void ASakuraPrototypeCharacter::ApplyVehicleSeatedPresentation()
{
...
void ASakuraPrototypeCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
