...
#include "SakuraControllerHotbarComponent.generated.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSakuraHotbarLayerChanged);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSakuraCancelRequested);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSakuraPrimaryRequested);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FOnSakuraPrimaryReleased);

/** The eight physical controls exposed by one cross-hotbar layer. */
UENUM(BlueprintType)
enum class ESakuraHotbarInput : uint8
{
	DPadUp,
	DPadRight,
	DPadDown,
...
UENUM(BlueprintType)
enum class ESakuraHotbarLayer : uint8
{
	None,
	LeftShoulder,
	RightShoulder,
...
UENUM(BlueprintType)
enum class ESakuraHotbarPresentation : uint8
{
	Hidden,
	DeveloperOnly,
	PlayerFacing
...
UENUM(BlueprintType)
enum class ESakuraHotbarCellShape : uint8
{
	Square,
	Circle
};
...
struct SAKURACORE_API FSakuraHotbarSlotAddress
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="OWS|Hotbar")
	ESakuraHotbarLayer Layer = ESakuraHotbarLayer::None;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="OWS|Hotbar")
	ESakuraHotbarInput Input = ESakuraHotbarInput::DPadUp;
};

DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnSakuraHotbarSlotPressed, FSakuraHotbarSlotAddress, Slot);
...
class SAKURACORE_API USakuraControllerHotbarComponent final : public UActorComponent
{
	GENERATED_BODY()

public:
	USakuraControllerHotbarComponent();
	...
	FOnSakuraHotbarLayerChanged OnHotbarLayerChanged;

	UPROPERTY(BlueprintAssignable, Category="OWS|Hotbar")
	FOnSakuraHotbarSlotPressed OnHotbarSlotPressed;
	...
	ESakuraHotbarPresentation Presentation = ESakuraHotbarPresentation::DeveloperOnly;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="OWS|Hotbar|Presentation")
	ESakuraHotbarCellShape CellShape = ESakuraHotbarCellShape::Square;
	...
	ESakuraHotbarLayer GetActiveLayer() const;
	...
	FOnSakuraCancelRequested OnCancelRequested;

	/** Generic R2 primary-hand/unarmed action seam. Gameplay assigns its behavior. */
	UPROPERTY(BlueprintAssignable, Category="OWS|Input")
	FOnSakuraPrimaryRequested OnPrimaryRequested;

	UPROPERTY(BlueprintAssignable, Category="OWS|Input")
	FOnSakuraPrimaryReleased OnPrimaryReleased;
...
private:
	...
	void Route(ESakuraHotbarInput Input);
	...
	ESakuraHotbarLayer FirstHeldShoulder = ESakuraHotbarLayer::None;
	ESakuraHotbarLayer ActiveLayer = ESakuraHotbarLayer::None;
};
