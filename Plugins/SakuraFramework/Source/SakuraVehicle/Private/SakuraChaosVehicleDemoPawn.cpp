...
#include "SakuraVehicleSimCurveWheelComponent.h"
#include "SakuraVehicleSimIgnitionEngineComponent.h"
#include "SakuraVehicleSimSuspensionComponent.h"
#include "SimModule/ModuleInput.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SakuraChaosVehicleDemoPawn)

namespace Sakura::Vehicle::Demo
{
...
ASakuraChaosVehicleDemoPawn::ASakuraChaosVehicleDemoPawn(
	const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	using namespace Sakura::Vehicle::Demo;
	...
	FSakuraVehicleSeatDefinition DriverSeat;
	...
	FSakuraVehicleSeatDefinition PassengerSeat;
	...
	FSakuraVehicleDoorDefinition LeftDoor;
	...
	FSakuraVehicleDoorDefinition RightDoor;
	...
	VehicleMesh->SetCollisionProfileName(TEXT("SakuraVehicle"));
	...
	InputRouter = CreateDefaultSubobject<USakuraChaosVehicleInputRouterComponent>(
		TEXT("SakuraInputRouter"));
	InputRouter->SetVehicleComponent(VehicleComponent);
	InputRouter->AddTickPrerequisiteActor(this);

	ThrottleShaper = CreateDefaultSubobject<UInputModifier_SakuraKinetiThrottle>(
		TEXT("SakuraThrottleShaper"));

	Chassis = CreateDefaultSubobject<UVehicleSimChassisComponent>(
		TEXT("VehicleSimChassis"));
	Chassis->SetupAttachment(VehicleMesh);

	Engine = CreateDefaultSubobject<USakuraVehicleSimIgnitionEngineComponent>(
	...
	FrontLeftSuspension =
		CreateDefaultSubobject<USakuraVehicleSimSuspensionComponent>(
	...
	FrontLeftWheel = CreateDefaultSubobject<USakuraVehicleSimCurveWheelComponent>(
	...
	FrontRightWheel = CreateDefaultSubobject<USakuraVehicleSimCurveWheelComponent>(
	...
	BackLeftWheel = CreateDefaultSubobject<USakuraVehicleSimCurveWheelComponent>(
		TEXT("VehicleSimWheel_BL"));
	BackLeftWheel->SetupAttachment(BackLeftSuspension);
	ConfigureWheel(BackLeftWheel, BackLeftBone, false);

	BackRightWheel = CreateDefaultSubobject<USakuraVehicleSimCurveWheelComponent>(
...
ASakuraChaosVehicleStockDemoPawn::ASakuraChaosVehicleStockDemoPawn(
	const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	ComparisonMode = ESakuraChaosVehicleDemoMode::StockBaseline;
	ApplyComparisonMode();
}

void ASakuraChaosVehicleDemoPawn::Tick(const float DeltaSeconds)
{
	...
	if (ComparisonMode == ESakuraChaosVehicleDemoMode::SakuraHybrid &&
...
void ASakuraChaosVehicleDemoPawn::RequestIgnitionStart()
{
...
bool ASakuraChaosVehicleDemoPawn::FindFacedDoor(
	...
	float& OutDistance) const
{
	...
	for (const FSakuraVehicleDoorDefinition& Door : DoorDefinitions)
	{
...
bool ASakuraChaosVehicleDemoPawn::SelectAvailableSeat(
	const FName DoorId,
...
{
	OutSeatId = NAME_None;
	const FSakuraVehicleDoorDefinition* Door = FindDoorDefinition(DoorId);
	...
	if (bAutoPlacementEnabled)
	{
		...
		{
			for (const FSakuraVehicleSeatDefinition& Seat : SeatDefinitions)
			{
		...
		{
			// A controller assigned outside Sakura's occupancy registry still owns
...
bool ASakuraChaosVehicleDemoPawn::OccupySeat(
	const FName SeatId,
	AActor* OccupantActor)
{
	...
	for (FSakuraVehicleSeatOccupancy& State : SeatOccupancy)
	{
...
bool ASakuraChaosVehicleDemoPawn::ReleaseSeat(AActor* OccupantActor)
{
	if (!HasAuthority() || !IsValid(OccupantActor))
	{
		return false;
...
bool ASakuraChaosVehicleDemoPawn::GetSeatWorldTransform(
	const FName SeatId,
	FTransform& OutTransform) const
{
	if (const FSakuraVehicleSeatDefinition* Seat = FindSeatDefinition(SeatId))
	{
...
bool ASakuraChaosVehicleDemoPawn::GetDoorWorldTransform(
	const FName DoorId,
	FTransform& OutTransform) const
{
	if (const FSakuraVehicleDoorDefinition* Door = FindDoorDefinition(DoorId))
	{
...
bool ASakuraChaosVehicleDemoPawn::IsControlSeat(const FName SeatId) const
{
	const FSakuraVehicleSeatDefinition* Seat = FindSeatDefinition(SeatId);
	return Seat != nullptr && Seat->bControlsVehicle;
}

FName ASakuraChaosVehicleDemoPawn::GetSeatForOccupant(
	const AActor* OccupantActor) const
{
	...
	for (const FSakuraVehicleSeatOccupancy& State : SeatOccupancy)
	{
		if (State.Occupant == OccupantActor)
		{
			return State.SeatId;
...
bool ASakuraChaosVehicleDemoPawn::HasControlSeatOccupant() const
{
	for (const FSakuraVehicleSeatDefinition& Seat : SeatDefinitions)
	{
...
bool ASakuraChaosVehicleDemoPawn::IsRecoveryOwner(
	const AActor* OccupantActor) const
{
	...
	if (GetController() != nullptr)
	{
		...
		// an actor in Sakura's control seat. Passengers cannot recover its vehicle.
		return false;
	}
...
void ASakuraChaosVehicleDemoPawn::BeginPlay()
{
	Super::BeginPlay();
...
void ASakuraChaosVehicleDemoPawn::GetLifetimeReplicatedProps(
	TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(ASakuraChaosVehicleDemoPawn, SeatOccupancy);
}

void ASakuraChaosVehicleDemoPawn::InitializeSeatOccupancy()
{
	if (!HasAuthority())
	{
		return;
	...
	for (const FSakuraVehicleSeatDefinition& Seat : SeatDefinitions)
	{
		...
		{
			FSakuraVehicleSeatOccupancy& State = SeatOccupancy.AddDefaulted_GetRef();
...
const FSakuraVehicleSeatDefinition*
ASakuraChaosVehicleDemoPawn::FindSeatDefinition(const FName SeatId) const
{
	return SeatDefinitions.FindByPredicate(
		[SeatId](const FSakuraVehicleSeatDefinition& Seat)
		{
			return Seat.SeatId == SeatId;
		});
}

const FSakuraVehicleDoorDefinition*
ASakuraChaosVehicleDemoPawn::FindDoorDefinition(const FName DoorId) const
{
	return DoorDefinitions.FindByPredicate(
		[DoorId](const FSakuraVehicleDoorDefinition& Door)
		{
			return Door.DoorId == DoorId;
		});
}

bool ASakuraChaosVehicleDemoPawn::IsSeatAvailable(
	const FName SeatId,
	const AActor* EnteringActor) const
{
	...
	const FSakuraVehicleSeatOccupancy* State = SeatOccupancy.FindByPredicate(
		[SeatId](const FSakuraVehicleSeatOccupancy& Candidate)
		{
	...
	FCollisionQueryParams QueryParams(
		SCENE_QUERY_STAT(SakuraVehicleSeatClearance),
		false,
		this);
...
void ASakuraChaosVehicleDemoPawn::OnRep_SeatOccupancy()
{
...
void ASakuraChaosVehicleDemoPawn::OnConstruction(const FTransform& Transform)
{
...
void ASakuraChaosVehicleDemoPawn::PawnClientRestart()
{
...
void ASakuraChaosVehicleDemoPawn::NotifyControllerChanged()
{
...
void ASakuraChaosVehicleDemoPawn::UnPossessed()
{
	ClearHeldInputs();
	SetParkedState(true);
	RemoveInputMapping();
...
void ASakuraChaosVehicleDemoPawn::SetupPlayerInputComponent(
	UInputComponent* PlayerInputComponent)
{
	...
	if (LoadedThrottleAction != nullptr)
	{
		EnhancedInput->BindAction(
			...
			this,
			&ASakuraChaosVehicleDemoPawn::OnThrottleInput);
		EnhancedInput->BindAction(
			LoadedThrottleAction,
			ETriggerEvent::Completed,
			this,
			&ASakuraChaosVehicleDemoPawn::OnThrottleReleased);
		EnhancedInput->BindAction(
			LoadedThrottleAction,
	...
	if (LoadedBrakeAction != nullptr)
	{
		EnhancedInput->BindAction(
			...
			ETriggerEvent::Triggered,
			this,
			&ASakuraChaosVehicleDemoPawn::OnBrakeInput);
		EnhancedInput->BindAction(
			LoadedBrakeAction,
			ETriggerEvent::Completed,
			this,
			&ASakuraChaosVehicleDemoPawn::OnBrakeReleased);
		EnhancedInput->BindAction(
			LoadedBrakeAction,
			ETriggerEvent::Canceled,
			this,
	...
	if (LoadedSteeringAction != nullptr)
	{
		EnhancedInput->BindAction(
			...
			ETriggerEvent::Triggered,
			this,
			&ASakuraChaosVehicleDemoPawn::OnSteeringInput);
		EnhancedInput->BindAction(
			LoadedSteeringAction,
			ETriggerEvent::Completed,
			this,
			&ASakuraChaosVehicleDemoPawn::OnSteeringReleased);
		EnhancedInput->BindAction(
			LoadedSteeringAction,
			ETriggerEvent::Canceled,
			this,
	...
	if (LoadedHandbrakeAction != nullptr)
	{
		EnhancedInput->BindAction(
			...
			ETriggerEvent::Triggered,
			this,
			&ASakuraChaosVehicleDemoPawn::OnHandbrakeInput);
		EnhancedInput->BindAction(
			LoadedHandbrakeAction,
			ETriggerEvent::Completed,
			this,
			&ASakuraChaosVehicleDemoPawn::OnHandbrakeReleased);
		EnhancedInput->BindAction(
			LoadedHandbrakeAction,
			ETriggerEvent::Canceled,
			this,
	...
	if (LoadedLookAction != nullptr)
	{
		EnhancedInput->BindAction(
			...
			ETriggerEvent::Triggered,
			this,
			&ASakuraChaosVehicleDemoPawn::OnLookInput);
	}

	InstallInputMapping();
}

void ASakuraChaosVehicleDemoPawn::EndPlay(
...
void ASakuraChaosVehicleDemoPawn::ApplyComparisonMode()
{
	const bool bEnableCurve =
		ComparisonMode == ESakuraChaosVehicleDemoMode::SakuraHybrid;
...
void ASakuraChaosVehicleDemoPawn::ApplyExampleAssets()
{
	USkeletalMeshComponent* VehicleMesh = GetSkeletalMeshComponent();
...
void ASakuraChaosVehicleDemoPawn::ConfigureSuspension(
...
void ASakuraChaosVehicleDemoPawn::ConfigureWheel(
...
void ASakuraChaosVehicleDemoPawn::LoadInputAssets()
{
...
void ASakuraChaosVehicleDemoPawn::InstallInputMapping()
{
	LoadInputAssets();
...
void ASakuraChaosVehicleDemoPawn::RemoveInputMapping()
{
...
void ASakuraChaosVehicleDemoPawn::ClearHeldInputs()
{
...
void ASakuraChaosVehicleDemoPawn::SetParkedState(const bool bNewParked)
{
...
void ASakuraChaosVehicleDemoPawn::UpdateParkedState(const float DeltaSeconds)
{
	if (!HasAuthority())
	{
		return;
...
void ASakuraChaosVehicleDemoPawn::PublishVehicleControls(
...
void ASakuraChaosVehicleDemoPawn::AlignParkedVehicleToGround()
{
	USkeletalMeshComponent* VehicleMesh = GetSkeletalMeshComponent();
	UWorld* World = GetWorld();
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(SakuraVehicleGroundAlign), false, this);
	QueryParams.AddIgnoredActor(this);
...
void ASakuraChaosVehicleDemoPawn::OnThrottleInput(
	const FInputActionValue& Value)
{
	RawThrottleInput = FMath::Clamp(Value.Get<float>(), 0.0f, 1.0f);
}

void ASakuraChaosVehicleDemoPawn::OnThrottleReleased(
	const FInputActionValue& Value)
{
	RawThrottleInput = 0.0f;
}

void ASakuraChaosVehicleDemoPawn::OnBrakeInput(const FInputActionValue& Value)
{
	BrakeInput = FMath::Clamp(Value.Get<float>(), 0.0f, 1.0f);
}

void ASakuraChaosVehicleDemoPawn::OnBrakeReleased(
	const FInputActionValue& Value)
{
	BrakeInput = 0.0f;
}

void ASakuraChaosVehicleDemoPawn::OnSteeringInput(
	const FInputActionValue& Value)
{
	SteeringInput = FMath::Clamp(Value.Get<float>(), -1.0f, 1.0f);
}

void ASakuraChaosVehicleDemoPawn::OnSteeringReleased(
	const FInputActionValue& Value)
{
	SteeringInput = 0.0f;
}

void ASakuraChaosVehicleDemoPawn::OnHandbrakeInput(
	const FInputActionValue& Value)
{
	bHandbrakeInput = Value.Get<bool>();
}

void ASakuraChaosVehicleDemoPawn::OnHandbrakeReleased(
	const FInputActionValue& Value)
{
	bHandbrakeInput = false;
}

void ASakuraChaosVehicleDemoPawn::OnReverseInput(
	const FInputActionValue& Value)
{
	bReverseInput = Value.Get<bool>();
}

void ASakuraChaosVehicleDemoPawn::OnReverseReleased(
	const FInputActionValue& Value)
{
	bReverseInput = false;
}

void ASakuraChaosVehicleDemoPawn::OnLookInput(const FInputActionValue& Value)
{
