...
DEFINE_LOG_CATEGORY_STATIC(LogSakuraMoverComparison, Log, All);

const FName ASakuraMoverComparisonPawn::ComparisonSaveSlot(
	TEXT("SakuraMoverComparisonCheckpoint"));

ASakuraMoverComparisonPawn::ASakuraMoverComparisonPawn(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	SaveRestoreComponent = CreateDefaultSubobject<USakuraMoverSaveRestoreComponent>(TEXT("MoverSaveRestore"));
...
void ASakuraMoverComparisonPawn::BeginPlay()
{
	Super::BeginPlay();
	...
	if (HasAuthority())
	{
		if (USaveManager* SaveManager = USaveManager::Get(this))
		{
			...
			SaveManager->EnsureActiveSlot(USakuraMoverComparisonSaveSlot::StaticClass(), true);
		}
	}

	if (IsLocallyControlled())
	{
		UE_LOG(LogSakuraMoverComparison, Display,
...
void ASakuraMoverComparisonPawn::PawnClientRestart()
{
	Super::PawnClientRestart();
	AddInputContexts();
}

void ASakuraMoverComparisonPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void ASakuraMoverComparisonPawn::AddInputContexts()
{
...
void ASakuraMoverComparisonPawn::OnMoveTriggered(const FInputActionValue& Value)
{
	CachedMoveInput = Value.Get<FVector2D>().GetClampedToMaxSize(1.0f);
}

void ASakuraMoverComparisonPawn::OnMoveCompleted(const FInputActionValue& Value)
{
	CachedMoveInput = FVector2D::ZeroVector;
}

void ASakuraMoverComparisonPawn::OnLookTriggered(const FInputActionValue& Value)
{
...
void ASakuraMoverComparisonPawn::OnJumpStarted(const FInputActionValue& Value)
{
	bJumpJustPressed = !bJumpPressed;
	bJumpPressed = true;
}

void ASakuraMoverComparisonPawn::OnJumpCompleted(const FInputActionValue& Value)
{
	bJumpPressed = false;
	bJumpJustPressed = false;
}

void ASakuraMoverComparisonPawn::ProduceInput_Implementation(
...
void ASakuraMoverComparisonPawn::RequestSaveComparisonCheckpoint()
{
	if (HasAuthority())
	{
...
void ASakuraMoverComparisonPawn::RequestLoadComparisonCheckpoint()
{
	if (HasAuthority())
	{
...
bool ASakuraMoverComparisonPawn::ServerSaveComparisonCheckpoint_Validate()
{
	return true;
}

void ASakuraMoverComparisonPawn::ServerSaveComparisonCheckpoint_Implementation()
{
	SaveComparisonCheckpointAuthority();
}

bool ASakuraMoverComparisonPawn::ServerLoadComparisonCheckpoint_Validate()
{
	return true;
}

void ASakuraMoverComparisonPawn::ServerLoadComparisonCheckpoint_Implementation()
{
	LoadComparisonCheckpointAuthority();
}

void ASakuraMoverComparisonPawn::SaveComparisonCheckpointAuthority()
{
	SetCheckpointResult(
...
void ASakuraMoverComparisonPawn::LoadComparisonCheckpointAuthority()
{
	SetCheckpointResult(
...
void ASakuraMoverComparisonPawn::HandleAutomaticRestoreEvaluated(
	const FName LoadedSlotName,
	const ESakuraGroundedMovementResult Result)
{
	if (HasAuthority())
	{
...
void ASakuraMoverComparisonPawn::SetCheckpointResult(
	const ESakuraGroundedMovementResult NewResult,
...
void ASakuraMoverComparisonPawn::OnRep_LastCheckpointResult()
{
	DisplayCheckpointResult(TEXT("Checkpoint"));
}

void ASakuraMoverComparisonPawn::DisplayCheckpointResult(const TCHAR* Operation) const
{
	...
	const FString Message = FString::Printf(
		...
		Operation,
		*ResultText.ToString());
	UE_LOG(LogSakuraMoverComparison, Display, TEXT("%s"), *Message);
	if (IsLocallyControlled() && GEngine)
	{
		GEngine->AddOnScreenDebugMessage(
			740079,
			5.0f,
			LastCheckpointResult == ESakuraGroundedMovementResult::Success
				? FColor::Green
				: FColor::Yellow,
			Message);
	}
...
void ASakuraMoverComparisonPawn::GetLifetimeReplicatedProps(
