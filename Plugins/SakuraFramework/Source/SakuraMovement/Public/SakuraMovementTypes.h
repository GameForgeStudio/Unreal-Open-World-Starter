...
#include "SakuraMovementTypes.generated.h"

/**
 * Result of validating or requesting Sakura's deliberately narrow grounded
 * Mover checkpoint seam.
 */
UENUM(BlueprintType)
enum class ESakuraGroundedMovementResult : uint8
{
...
struct SAKURAMOVEMENT_API FSakuraGroundedMovementCheckpoint
{
	GENERATED_BODY()

	UPROPERTY(SaveGame, VisibleAnywhere, BlueprintReadOnly, Category = "Sakura|Movement")
...
namespace UE::SakuraMovement
{
	...
	SAKURAMOVEMENT_API bool IsCheckpointTransformValid(const FTransform& Transform);

	SAKURAMOVEMENT_API ESakuraGroundedMovementResult ValidateGroundedCapture(
		const FGroundedCaptureValidationInput& Input);

	SAKURAMOVEMENT_API ESakuraGroundedMovementResult ValidateGroundedRestore(
