...
param(
    ...
    [ValidateNotNullOrEmpty()]
    [string] $ScratchRoot = (Join-Path ([IO.Path]::GetTempPath()) 'OWSOpenWorldLab')
)
...
$ProjectFile = Join-Path $ProjectRoot 'OWS.uproject'
$BuildBat = Join-Path $EngineRoot 'Engine\Build\BatchFiles\Build.bat'
$Editor = Join-Path $EngineRoot 'Engine\Binaries\Win64\UnrealEditor.exe'
$EditorCmd = Join-Path $EngineRoot 'Engine\Binaries\Win64\UnrealEditor-Cmd.exe'
$BuildVersionFile = Join-Path $EngineRoot 'Engine\Build\Build.version'
$SourcePackage = '/Game/OWSPrototype/Maps/L_OWSTestLab'
$TargetPackage = '/Game/OWSPrototype/Maps/L_OWSTestLab_WP'
$SourceMap = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab.umap'
$TargetMap = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab_WP.umap'
$DefaultEngineIni = Join-Path $ProjectRoot 'Config\DefaultEngine.ini'
$LevelIni = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab.ini'
$ExternalActors = Join-Path $ProjectRoot 'Content\__ExternalActors__\OWSPrototype\Maps\L_OWSTestLab_WP'
$ExternalObjects = Join-Path $ProjectRoot 'Content\__ExternalObjects__\OWSPrototype\Maps\L_OWSTestLab_WP'
$DataLayers = Join-Path $ProjectRoot 'Content\DataLayers\L_OWSTestLab_WP'
$GeneratedAssets = Join-Path $ProjectRoot 'Content\OWSPrototype\Terrain\OpenWorldGenerated'
$HLODInstanced = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab_HLODLayer_Instanced.uasset'
$HLODMerged = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab_HLODLayer_Merged.uasset'
$TargetHLODInstanced = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab_WP_HLODLayer_Instanced.uasset'
$TargetHLODMerged = Join-Path $ProjectRoot 'Content\OWSPrototype\Maps\L_OWSTestLab_WP_HLODLayer_Merged.uasset'
...
function Assert-OutputsAbsent {
    ...
    $siblings = @(
        Get-ChildItem -LiteralPath $safeMapDirectory -Force -ErrorAction Stop |
            Where-Object {
                $_.Name.StartsWith('L_OWSTestLab_WP', [StringComparison]::OrdinalIgnoreCase)
            }
    )
...
function Assert-EditorLogSentinel {
    ...
    $fatalMatches = @(
        Select-String -LiteralPath $Log -Pattern 'LogWindows: Error:|OWS editor generation failed:'
    )
...
function Get-TaskOwnedContentOutputs {
    ...
    if (Test-Path -LiteralPath $safeMapDirectory -PathType Container) {
        foreach ($item in Get-ChildItem -LiteralPath $safeMapDirectory -Force -ErrorAction Stop) {
            if ($item.Name.StartsWith('L_OWSTestLab_WP', [StringComparison]::OrdinalIgnoreCase)) {
...
try {
    ...
    $buildCommand = '"{0}" OWSEditor Win64 Development -Project="{1}" -NoHotReload -NoEngineChanges -NoUBA -NoXGE -NoFASTBuild -NoSNDBS -NoLog' -f $BuildBat, $ProjectFile
    Invoke-TrackedBatch -Command $buildCommand -Label 'OWS editor build'
    Assert-NoUnrealBuildTool
    Assert-NoEditor
    Assert-OutputsAbsent
    ...
    Invoke-TrackedProcess -File $Editor -Arguments (@(
        $ProjectFile,
        $TargetPackage,
        '-OWSGenerateOpenWorldLabEditor',
        "-OWSOpenWorldSeed=$Seed",
        "-AbsLog=$generateLog"
    ) + @($commonArguments | Where-Object { $_ -ne '-AllowCommandletRendering' })) -Label 'OWS open-world generation'
    Assert-EditorLogSentinel -Log $generateLog -Sentinel 'OWS_EDITOR_GENERATION_SUCCESS' -Label 'OWS open-world generation'

    $beforeValidation = @(Get-GeneratedSnapshot)
    Invoke-TrackedProcess -File $EditorCmd -Arguments (@(
        $ProjectFile,
        '-run=OWSGenerateOpenWorldLab',
        $TargetPackage,
        '-ValidateOnly',
        "-Seed=$Seed",
        "-AbsLog=$validateLog"
    ) + $commonArguments) -Label 'OWS open-world validation'
    Assert-LogSentinel -Log $validateLog -Sentinel 'OWS_VALIDATION_SUCCESS' -Label 'OWS open-world validation'
    ...
    if (-not $success -and $contentMutationStarted) {
        if ($rollbackIsSafe) {
            ...
            $cleanupErrors.Add("Rollback could not inspect/remove possible companion conversion assets with prefix 'L_OWSTestLab_WP' under '$MapDirectory'.")
            foreach ($ancestor in $absentOutputAncestors) {
...
Write-Host 'OWS open-world lab generation and fresh-process validation succeeded.' -ForegroundColor Green
