...
#include "OWSTestLabPhysicsProp.h"
...
AOWSTestLabEnvironment::AOWSTestLabEnvironment()
{
	using namespace OWS::TestLab;
...
void AOWSTestLabEnvironment::BeginPlay()
{
	Super::BeginPlay();
...
void AOWSTestLabEnvironment::EndPlay(
...
AOWSTestLabPhysicsProp* AOWSTestLabEnvironment::SpawnPhysicsProp(
	...
	const float Restitution)
{
	...
	AOWSTestLabPhysicsProp* Prop = GetWorld()->SpawnActor<AOWSTestLabPhysicsProp>(
		AOWSTestLabPhysicsProp::StaticClass(),
		WorldTransform,
		SpawnParameters);
	if (!Prop)
	{
...
void AOWSTestLabEnvironment::SpawnRuntimeShowcase()
{
	using namespace OWS::TestLab;
	...
	SpawnPhysicsProp(
		TEXT("OWS.Physics.Restitution.Low"),
		BasicSphereMesh,
		...
		0.02f);
	SpawnPhysicsProp(
		TEXT("OWS.Physics.Restitution.High"),
		BasicSphereMesh,
	...
	if (GetWorld())
	{
		...
		for (const float X : {6650.0f, 7450.0f})
		{
			...
			AOWSTestLabRagdollTarget* Target = GetWorld()->SpawnActor<AOWSTestLabRagdollTarget>(
				AOWSTestLabRagdollTarget::StaticClass(),
...
void AOWSTestLabEnvironment::OnConstruction(const FTransform& Transform)
{
...
void AOWSTestLabEnvironment::ApplyColor(
	UInstancedStaticMeshComponent* Component,
