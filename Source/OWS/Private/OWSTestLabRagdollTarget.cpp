...
#include "OWSTestLabRagdollTarget.h"
...
AOWSTestLabRagdollTarget::AOWSTestLabRagdollTarget()
{
	...
	ImpactCapsule->OnComponentHit.AddDynamic(this, &AOWSTestLabRagdollTarget::HandleImpact);
	Tags.Add(TEXT("OWS.Physics.RagdollTarget"));
}

void AOWSTestLabRagdollTarget::BeginPlay()
{
	Super::BeginPlay();
}

void AOWSTestLabRagdollTarget::Tick(const float DeltaSeconds)
{
...
void AOWSTestLabRagdollTarget::ArmRagdollBeforeVehicleContact()
{
	...
	FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(OWSRagdollVehicleApproach), false, this);
	...
	for (const FOverlapResult& Overlap : Overlaps)
	{
		...
		if (!OtherActor || !OtherComponent
			|| !OtherActor->FindComponentByClass<UOWSStockVehicleInteractionComponent>())
		{
			continue;
		}
...
void AOWSTestLabRagdollTarget::GetLifetimeReplicatedProps(
	TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME(AOWSTestLabRagdollTarget, bRagdollActive);
	DOREPLIFETIME(AOWSTestLabRagdollTarget, ReplicatedImpactImpulse);
	DOREPLIFETIME(AOWSTestLabRagdollTarget, ReplicatedImpactPoint);
}

void AOWSTestLabRagdollTarget::HandleImpact(
...
void AOWSTestLabRagdollTarget::MulticastActivateRagdoll_Implementation(
...
void AOWSTestLabRagdollTarget::OnRep_RagdollState()
{
	if (bRagdollActive)
	{
...
void AOWSTestLabRagdollTarget::ActivateRagdoll(
	const FVector& ImpactImpulse,
	const FVector& ImpactPoint)
{
	if (bRagdollApplied)
