...
DEFINE_LOG_CATEGORY_STATIC(LogSakuraOpenWorldGenerator, Log, All);

namespace SakuraOpenWorld
{
	...
	const TCHAR* const ExpectedTargetMapPackage = TEXT("/Game/SakuraPrototype/Maps/L_SakuraTestLab_WP");
	const TCHAR* const GeneratedAssetRoot = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated");
	const TCHAR* const TerrainMaterialPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/M_SakuraOpenWorldTerrain");
	const TCHAR* const GrassLayerPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/LI_Grass");
	const TCHAR* const AsphaltLayerPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/LI_Asphalt");
	const TCHAR* const GravelLayerPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/LI_Gravel");
	const TCHAR* const DirtLayerPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/LI_Dirt");
	const TCHAR* const CityMaterialPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/M_CityBlock");
	const TCHAR* const ConcreteMaterialPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/M_Concrete");
	const TCHAR* const MarkerMaterialPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/M_RouteMarker");
	const TCHAR* const BridgeMaterialPackage = TEXT("/Game/SakuraPrototype/Terrain/OpenWorldGenerated/M_BridgeDeck");

	const FName GeneratedTag(TEXT("Sakura.OpenWorld.Generated"));
	const FName LandscapeTag(TEXT("Sakura.OpenWorld.Landscape.V1"));
	const FName CampusManagerTag(TEXT("Sakura.OpenWorld.CampusManager"));
	const FName CityBlockTag(TEXT("Sakura.OpenWorld.CityBlock"));
	const FName RouteMarkerTag(TEXT("Sakura.OpenWorld.RouteMarker"));
	const FName DistrictSignTag(TEXT("Sakura.OpenWorld.DistrictSign"));
	const FName BridgeTag(TEXT("Sakura.OpenWorld.Bridge"));
	const FName RecoveryAnchorTag(TEXT("Sakura.OpenWorld.RecoveryAnchor"));
	const FName MultiplayerStartTag(TEXT("Sakura.OpenWorld.MultiplayerStart"));
	...
	static bool SpawnDistrictSign(
		...
		float YawDegrees)
	{
		const FName Folder(TEXT("SakuraOpenWorld/Signs"));
		const FString BackingName = FString::Printf(TEXT("Sakura_SignBacking_%s"), *Identifier);
		...
		const FString TextActorName = FString::Printf(TEXT("Sakura_SignText_%s"), *Identifier);
		FActorSpawnParameters SpawnParameters;
		...
		TextActor->SetActorLabel(FString::Printf(TEXT("Sakura Sign - %s"), *Text));
		...
		uint32 Seed)
	{
		...
		const FName Folder(TEXT("SakuraOpenWorld/Urban/Blocks"));
			...
			for (int32 BlockX = 0; BlockX < 5; ++BlockX)
			{
				...
				for (int32 Quadrant = 0; Quadrant < 4; ++Quadrant)
				{
					...
					const FString Name = FString::Printf(
						TEXT("Sakura_CityBlock_%02d_%02d_%c"),
						BlockX,
						BlockY,
						TEXT('A') + Quadrant);
					if (!SpawnBox(
	...
	{
		const FName Folder(TEXT("SakuraOpenWorld/Urban/Bridge"));
		...
		if (!SpawnSegment(World, CubeMesh, BridgeMaterial, TEXT("Sakura_Bridge_SouthRamp"), RampSouthStart, RampSouthEnd, 14.0f, 0.5f, BridgeTag, Folder)
			|| !SpawnSegment(World, CubeMesh, BridgeMaterial, TEXT("Sakura_Bridge_Deck"), RampSouthEnd, DeckNorth, 14.0f, 0.5f, BridgeTag, Folder)
			|| !SpawnSegment(World, CubeMesh, BridgeMaterial, TEXT("Sakura_Bridge_NorthRamp"), DeckNorth, RampNorthEnd, 14.0f, 0.5f, BridgeTag, Folder))
		{
			return false;
		}

		if (!SpawnBox(World, CubeMesh, ConcreteMaterial, TEXT("Sakura_Bridge_SouthAbutment"), FVector(1900.0f, 1690.0f, 11.5f), FVector(14.0f, 2.0f, 7.0f), FRotator::ZeroRotator, BridgeTag, Folder)
			|| !SpawnBox(World, CubeMesh, ConcreteMaterial, TEXT("Sakura_Bridge_NorthAbutment"), FVector(1900.0f, 2110.0f, 11.5f), FVector(14.0f, 2.0f, 7.0f), FRotator::ZeroRotator, BridgeTag, Folder))
		{
			return false;
		}
		...
		for (int32 Side = -1; Side <= 1; Side += 2)
		{
			...
			if (!SpawnSegment(World, CubeMesh, ConcreteMaterial,
				FString::Printf(TEXT("Sakura_Bridge_Rail_%s"), Side < 0 ? TEXT("West") : TEXT("East")),
	...
	{
		const FName Folder(TEXT("SakuraOpenWorld/Routes/Markers"));
		int32 MarkerIndex = 0;
		auto SpawnMarker = [&](const FVector2D& XY, const FString& Prefix) -> bool
		{
			...
			return SpawnBox(
				...
				CubeMesh,
				MarkerMaterial,
				FString::Printf(TEXT("Sakura_%s_Marker_%03d"), *Prefix, MarkerIndex++),
				FVector(XY.X, XY.Y, Z),
		...
		UMaterialInterface* MarkerMaterial)
	{
		...
		{
			FActorSpawnParameters SpawnParameters;
			const FString ActorName = FString::Printf(TEXT("Sakura_RecoveryAnchor_%02d"), Index);
			SpawnParameters.Name = FName(*ActorName);
			SpawnParameters.OverrideActorGuid = StableActorGuid(ActorName);
			SpawnParameters.NameMode =
				FActorSpawnParameters::ESpawnActorNameMode::Required_ErrorAndReturnNull;
			...
			Anchor->SetActorLabel(FString::Printf(TEXT("Sakura Recovery Anchor %02d"), Index));
			Anchor->SetFolderPath(TEXT("SakuraOpenWorld/Recovery"));
	...
	static bool SpawnMultiplayerStarts(UWorld* World)
	{
		...
		{
			FActorSpawnParameters SpawnParameters;
			const FString ActorName = FString::Printf(TEXT("Sakura_MultiplayerStart_%02d"), Index + 1);
			SpawnParameters.Name = FName(*ActorName);
			SpawnParameters.OverrideActorGuid = StableActorGuid(ActorName);
			SpawnParameters.NameMode =
				FActorSpawnParameters::ESpawnActorNameMode::Required_ErrorAndReturnNull;
			...
			Start->SetActorLabel(FString::Printf(TEXT("Sakura Multiplayer Start %02d"), Index + 1));
			Start->SetFolderPath(TEXT("SakuraOpenWorld/PlayerStarts"));
	...
	static bool MakeCampusManagerAlwaysLoaded(UWorld* World, FString& OutError)
	{
		...
		Params.ActorClasses = { ASakuraTestLabEnvironment::StaticClass() };
		...
		FWorldPartitionHelpers::ForEachActorWithLoading(
			...
			{
				if (ASakuraTestLabEnvironment* Manager = Cast<ASakuraTestLabEnvironment>(ActorDescInstance->GetActor()))
				{
					Manager->Modify();
		...
		if (ManagerCount != 1)
		{
			OutError = FString::Printf(TEXT("Expected exactly one ASakuraTestLabEnvironment campus manager; found %d."), ManagerCount);
			return false;
		}
		return true;
	}
		...
		FString& OutError)
	{
		...
		UE_LOG(LogSakuraOpenWorldGenerator, Display,
	...
	{
		FActorSpawnParameters SpawnParameters;
		SpawnParameters.Name = TEXT("SakuraOpenWorldLandscape");
		SpawnParameters.OverrideActorGuid = StableActorGuid(TEXT("SakuraOpenWorldLandscape"));
		SpawnParameters.NameMode =
			FActorSpawnParameters::ESpawnActorNameMode::Required_ErrorAndReturnNull;
		SpawnParameters.ObjectFlags |= RF_Transactional;
		...
		Landscape->SetActorLabel(TEXT("Sakura Open World Landscape 8129"));
		...
		Landscape->Tags.AddUnique(FName(*FString::Printf(TEXT("Sakura.OpenWorld.Seed.%u"), Seed)));
		...
		Landscape->Import(
			FGuid::NewDeterministicGuid(TEXT("Sakura.OpenWorld.LandscapeGuid"), Seed),
			0,
			0,
			InitialQuads,
			InitialQuads,
	...
	static bool ValidateStaticMeshCollisionQueries(UWorld* World, const AStaticMeshActor* MeshActor, FString& OutError)
	{
		...
		{
			FHitResult Hit;
			FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(SakuraGeneratedGeometryValidation), false);
		...
		FString& OutError)
	{
		...
		for (const FTerrainSample& Sample : Samples)
		{
			...
			if (Sample.bRequireLandscapeTrace)
			{
				...
				{
					FHitResult Hit;
					FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(SakuraLandscapeValidation), false);
		...
		ASakuraTestLabEnvironment* CampusManager = nullptr;
		for (TActorIterator<ASakuraTestLabEnvironment> It(World); It; ++It)
		{
		...
		FCollisionQueryParams FoundationQuery(SCENE_QUERY_STAT(SakuraCampusFoundationValidation), false);
		...
		FCollisionQueryParams UnderlayQuery(SCENE_QUERY_STAT(SakuraCampusUnderlayValidation), false);
	...
	static bool ValidateGeneratedWorld(UWorld* World, uint32 ExpectedSeed, FString& OutError)
	{
		...
		const FName ExpectedSeedTag(*FString::Printf(TEXT("Sakura.OpenWorld.Seed.%u"), ExpectedSeed));
		...
		ActorParams.ActorClasses = {
			...
			ASakuraTestLabEnvironment::StaticClass()
		};
...
USakuraGenerateOpenWorldLabCommandlet::USakuraGenerateOpenWorldLabCommandlet()
{
	IsClient = false;
	IsServer = false;
	IsEditor = true;
...
bool USakuraGenerateOpenWorldLabCommandlet::GenerateEditorWorld(UWorld* World, uint32 Seed, FString& OutError)
{
	if (!World || !World->GetOutermost()->GetName().Equals(
		SakuraOpenWorld::ExpectedTargetMapPackage,
	...
	return SakuraOpenWorld::GenerateWorld(
		World,
		SakuraOpenWorld::ExpectedTargetMapPackage,
		Seed,
		OutError);
...
int32 USakuraGenerateOpenWorldLabCommandlet::Main(const FString& Params)
{
	...
	{
		UE_LOG(LogSakuraOpenWorldGenerator, Error,
	...
	if (Tokens.Num() != 1)
	{
		UE_LOG(LogSakuraOpenWorldGenerator, Error, TEXT("Specify exactly one target map package token."));
		return 1;
	}

	FString MapPackageName = Tokens[0];
	if (!FPackageName::IsValidLongPackageName(MapPackageName)
		|| !MapPackageName.EndsWith(SakuraOpenWorld::RequiredMapSuffix, ESearchCase::CaseSensitive)
		|| !MapPackageName.Equals(SakuraOpenWorld::ExpectedTargetMapPackage, ESearchCase::CaseSensitive))
	{
		UE_LOG(LogSakuraOpenWorldGenerator, Error,
	...
	uint32 Seed = SakuraOpenWorld::DefaultSeed;
	if (const FString* SeedString = ParameterValues.Find(TEXT("Seed")))
	{
		if (!LexTryParseString(Seed, **SeedString))
		{
			UE_LOG(LogSakuraOpenWorldGenerator, Error, TEXT("Invalid unsigned -Seed value '%s'."), **SeedString);
			return 1;
		}
	}
	...
	if (!World)
	{
		UE_LOG(LogSakuraOpenWorldGenerator, Error, TEXT("Could not load and initialize map %s."), *MapPackageName);
		return 1;
	}
	...
	if (bGenerate)
	{
		bSucceeded = SakuraOpenWorld::GenerateWorld(World, MapPackageName, Seed, Error);
		if (bSucceeded)
		{
			UE_LOG(LogSakuraOpenWorldGenerator, Display, TEXT("SAKURA_GENERATION_SUCCESS"));
		}
	}
	else if (bValidateOnly)
	{
		bSucceeded = SakuraOpenWorld::ValidateGeneratedWorld(World, Seed, Error);
		if (bSucceeded)
		{
			UE_LOG(LogSakuraOpenWorldGenerator, Display, TEXT("SAKURA_VALIDATION_SUCCESS"));
		}
	}
	else
	{
		bSucceeded = SakuraOpenWorld::RepairGeneratedLandscapeMaterials(World, Error);
		if (bSucceeded)
		{
			UE_LOG(LogSakuraOpenWorldGenerator, Display, TEXT("SAKURA_MATERIAL_REPAIR_SUCCESS"));
		}
	}

	if (!bSucceeded)
	{
		UE_LOG(LogSakuraOpenWorldGenerator, Error, TEXT("%s"), *Error);
		return 1;
	}
	return 0;
}
