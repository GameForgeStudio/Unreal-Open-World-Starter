...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SakuraTestLabPlayerController)

namespace Sakura::TestLab::PlayerController
{
	const FString StandardModePath(
		TEXT("/Script/SakuraUnrealPrototype.SakuraTestLabGameMode"));
	const FString MoverModePath(
		TEXT("/Script/SakuraUnrealPrototype.SakuraTestLabMoverGameMode"));

}

ASakuraTestLabPlayerController::ASakuraTestLabPlayerController()
{
	bShowMouseCursor = false;
}

void ASakuraTestLabPlayerController::BeginPlay()
{
	Super::BeginPlay();
...
void ASakuraTestLabPlayerController::PlayerTick(const float DeltaTime)
{
	...
	{
		const ASakuraPrototypeCharacter* Occupant = GetHomeCharacterForHUD();
		const USakuraStockVehicleInteractionComponent* Interaction =
			AssociatedVehicle->FindComponentByClass<USakuraStockVehicleInteractionComponent>();
	...
	if (PawnZ < NoGroundRecoveryActivationZ && ControlledPawn->GetVelocity().Z < -50.0f)
	{
		if (UWorld* World = GetWorld())
		{
			FCollisionQueryParams QueryParams(
				SCENE_QUERY_STAT(SakuraNoGroundRecovery),
				false,
				ControlledPawn);
...
bool ASakuraTestLabPlayerController::InputKey(const FInputKeyEventArgs& Params)
{
...
void ASakuraTestLabPlayerController::SetupInputComponent()
{
...
void ASakuraTestLabPlayerController::RequestActivate()
{
...
void ASakuraTestLabPlayerController::GetLifetimeReplicatedProps(
	TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);
	DOREPLIFETIME_CONDITION(ASakuraTestLabPlayerController, HomeCharacter, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(ASakuraTestLabPlayerController, OccupiedVehicle, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(ASakuraTestLabPlayerController, OccupiedSeatId, COND_OwnerOnly);
	DOREPLIFETIME_CONDITION(ASakuraTestLabPlayerController, EnteredDoorId, COND_OwnerOnly);
...
ASakuraTestLabPlayerController::GetHomeCharacterForHUD() const
{
	if (ASakuraPrototypeCharacter* PossessedCharacter =
		Cast<ASakuraPrototypeCharacter>(GetPawn()))
	{
...
APawn*
ASakuraTestLabPlayerController::GetCurrentLabVehicle() const
{
	...
	return ControlledPawn != nullptr &&
		ControlledPawn->FindComponentByClass<USakuraStockVehicleInteractionComponent>() != nullptr
		? ControlledPawn
		: nullptr;
}

bool ASakuraTestLabPlayerController::IsCurrentVehicleDriver() const
{
	const APawn* Vehicle = GetCurrentLabVehicle();
	const USakuraStockVehicleInteractionComponent* Interaction = Vehicle != nullptr
		? Vehicle->FindComponentByClass<USakuraStockVehicleInteractionComponent>()
		: nullptr;
...
bool ASakuraTestLabPlayerController::IsInMoverMode() const
{
	using namespace Sakura::TestLab::PlayerController;
...
float ASakuraTestLabPlayerController::GetNearestEnterableVehicleDistance() const
{
	float Distance = -1.0f;
...
void ASakuraTestLabPlayerController::RequestToggleVehicle()
{
...
void ASakuraTestLabPlayerController::RequestToggleMoverMode()
{
	if (HasAuthority())
	{
...
void ASakuraTestLabPlayerController::ServerToggleVehicle_Implementation(
	const bool bAutoPlacementEnabled)
{
	ToggleVehicleAuthority(bAutoPlacementEnabled);
}

void ASakuraTestLabPlayerController::ServerToggleMoverMode_Implementation()
{
	ToggleMoverModeAuthority();
}

void ASakuraTestLabPlayerController::ToggleVehicleAuthority(
	const bool bAutoPlacementEnabled)
{
	if (!HasAuthority() || IsInMoverMode())
	{
	...
	if (APawn* CurrentVehicle = GetCurrentLabVehicle())
	{
		USakuraStockVehicleInteractionComponent* Interaction =
			CurrentVehicle->FindComponentByClass<USakuraStockVehicleInteractionComponent>();
...
void ASakuraTestLabPlayerController::ToggleMoverModeAuthority()
{
	using namespace Sakura::TestLab::PlayerController;
...
APawn*
ASakuraTestLabPlayerController::FindNearestEnterableVehicle(
	const bool bAutoPlacementEnabled,
	float* OutDistance,
	FName* OutDoorId,
	FName* OutSeatId) const
{
	...
	if (IsInMoverMode() || IsValid(OccupiedVehicle) ||
		(GetPawn() != nullptr &&
			GetPawn()->FindComponentByClass<USakuraStockVehicleInteractionComponent>() != nullptr))
	{
		return nullptr;
	}

	const ASakuraPrototypeCharacter* PrototypeCharacter =
		Cast<ASakuraPrototypeCharacter>(GetPawn());
	UWorld* World = GetWorld();
	...
	USakuraStockVehicleInteractionComponent* BestInteraction = nullptr;
	...
	FCollisionQueryParams NearbyQuery(
		SCENE_QUERY_STAT(SakuraNearbyVehicleBroadphase),
		false,
		PrototypeCharacter);
	...
	for (const FOverlapResult& Overlap : NearbyOverlaps)
	{
		if (APawn* Candidate = Cast<APawn>(Overlap.GetActor());
			Candidate != nullptr &&
			Candidate->FindComponentByClass<USakuraStockVehicleInteractionComponent>() != nullptr)
		{
	...
	for (APawn* Candidate : NearbyVehicles)
	{
		...
		USakuraStockVehicleInteractionComponent* CandidateInteraction =
			Candidate->FindComponentByClass<USakuraStockVehicleInteractionComponent>();
		...
		FCollisionQueryParams VisibilityQuery(
			SCENE_QUERY_STAT(SakuraVehicleDoorVisibility),
			false,
			PrototypeCharacter);
...
bool ASakuraTestLabPlayerController::EnterVehicleAuthority(
	ASakuraPrototypeCharacter& PrototypeCharacter,
	APawn& Vehicle,
	USakuraStockVehicleInteractionComponent& Interaction,
	const FName DoorId,
...
bool ASakuraTestLabPlayerController::EnterPassengerSeatAuthority(
	ASakuraPrototypeCharacter& PrototypeCharacter,
	APawn& Vehicle,
	USakuraStockVehicleInteractionComponent& Interaction,
	const FName DoorId,
...
bool ASakuraTestLabPlayerController::ExitVehicleAuthority(
	APawn& Vehicle,
	USakuraStockVehicleInteractionComponent& Interaction)
{
	ASakuraPrototypeCharacter* PrototypeCharacter = HomeCharacter.Get();
...
bool ASakuraTestLabPlayerController::ExitPassengerSeatAuthority(
	APawn& Vehicle,
	USakuraStockVehicleInteractionComponent& Interaction)
{
	ASakuraPrototypeCharacter* PrototypeCharacter = HomeCharacter.Get();
...
bool ASakuraTestLabPlayerController::PlaceHomeCharacterBesideVehicle(
	ASakuraPrototypeCharacter& PrototypeCharacter,
	const APawn& Vehicle,
	const USakuraStockVehicleInteractionComponent& Interaction,
...
bool ASakuraTestLabPlayerController::IsVehicleAutoPlacementEnabled() const
{
	if (GEngine != nullptr)
	{
		if (const USakuraUserSettings* Settings =
			Cast<USakuraUserSettings>(GEngine->GetGameUserSettings()))
		{
...
void ASakuraTestLabPlayerController::UpdateRecoveryCheckpoint(const float DeltaTime)
{
	...
	if (ASakuraPrototypeCharacter* PrototypeCharacter =
		Cast<ASakuraPrototypeCharacter>(GetPawn()))
	{
...
void ASakuraTestLabPlayerController::RecoverFallenPawn()
{
	ASakuraPrototypeCharacter* PrototypeCharacter =
		Cast<ASakuraPrototypeCharacter>(GetPawn());
...
void ASakuraTestLabPlayerController::RestoreAfterVehicleLossAuthority()
{
	if (!HasAuthority())
	{
		return;
