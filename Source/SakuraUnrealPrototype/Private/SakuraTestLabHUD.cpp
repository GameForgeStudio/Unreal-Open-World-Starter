...
#include UE_INLINE_GENERATED_CPP_BY_NAME(SakuraTestLabHUD)

namespace Sakura::TestLab::HUD
{
...
void ASakuraTestLabHUD::DrawHUD()
{
	Super::DrawHUD();
	...
	using namespace Sakura::TestLab::HUD;

	const ASakuraTestLabPlayerController* LabController =
		Cast<ASakuraTestLabPlayerController>(PlayerOwner);
	if (LabController == nullptr)
	{
		return;
	}

	const ASakuraMoverComparisonPawn* MoverPawn =
		Cast<ASakuraMoverComparisonPawn>(LabController->GetPawn());
	...
	DrawLabLine(
		bMoverMode
			? TEXT("SAKURA TEST LAB  |  ISOLATED MOVER MODE")
	...
	const ASakuraPrototypeCharacter* HomeCharacter =
	...
	else
	{
		...
		if (const USakuraPrototypeAttributeSet* Attributes =
		...
		if (USakuraPrototypeInventorySystemComponent* Inventory =
			HomeCharacter->FindComponentByClass<USakuraPrototypeInventorySystemComponent>())
		{
	...
	if (Vehicle != nullptr)
	{
		if (LabController->IsCurrentVehicleDriver())
		{
			...
			DrawLabLine(
				Interaction != nullptr && Interaction->GetKinetiForgeDriveAssembly() != nullptr
					? TEXT("Stock KinetiForge physics  |  Sakura Forza-style input/park lifecycle only  |  F also exits")
					: TEXT("Epic stock Chaos physics  |  Sakura Forza-style input/park lifecycle only  |  F also exits"),
				TextX,
				Y,
				FLinearColor(0.75f, 0.9f, 1.0f));
		}
		...
		{
			DrawLabLine(
				(Vehicle->FindComponentByClass<USakuraStockVehicleInteractionComponent>() != nullptr &&
					Vehicle->FindComponentByClass<USakuraStockVehicleInteractionComponent>()->HasControlSeatOccupant()) ||
...
void ASakuraTestLabHUD::DrawLabLine(
	const FString& Text,
	const float X,
	float& Y,
...
void ASakuraTestLabHUD::DrawInteractionPrompt(
	const FString& Text,
